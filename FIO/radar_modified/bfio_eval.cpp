...
int BFIO::eval(const CpxNumMat& f, CpxNumMat& u)
{
    ...
    ztran(mats(0), tmats(0));
    ztran(mats(1), tmats(1));
    
    CpxNumMat tdir;
    ztran(dir, tdir);
    
    int NG = _EPS; // what is this
    
    
    
    
	int tree_length = int(round(log(double(N))/log(2)));
	int SL = tree_length-3;
	int EL = 3;
	int tree_middle = int(floor(tree_length/2.0));
	int nz = pow2(EL);
	int zB = N/nz;
    ...
    for(int z2=0; z2<nz; z2++)
    {
        ...
        for(int ell=SL; ell>=tree_middle; ell--)
        {
            int nk = pow2(ell);
            int nx = N/nk;
            int kB = N/nk;
            ...
            for(int k2=k2stt/kB; k2<k2end/kB; k2++)
            {
                ...
                for(int x2=0; x2<nx; x2++)
                {
                    ...
                    if(ell==SL)
                    {
                        ...
                        //transform
                        CpxNumMat tmp(NG,kB);
                        setvalue(tmp, cpx(0,0));
                        zgemm(1, dir, ext, 0, tmp);
                        zgemm(1, tmp, tdir, 1, all);
                    }
                    else
                    {
                        int p1 = int(floor(x1/2));
                        int p2 = int(floor(x2/2));
                        ...
                        for(int a2=0; a2<2; a2++)
                        {
                            ...
                            CpxNumMat tmp(NG,NG);
                            setvalue(tmp,cpx(0,0));
                            zgemm(1, mats(a1), ext, 0, tmp);
                            zgemm(1, tmp, tmats(a2), 1, all);
                        }
                    }
        ...
        for(int k2=k2stt/kB; k2<k2end/kB; k2++)
        {
            ...
            for(int x2=0; x2<nx; x2++)
            {
                ...
                CpxNumMat evl(NG*NG,NG*NG);
                kernel(N, trg, src, evl);
                CpxNumVec den(NG*NG, true, NOW(k1,k2)(x1,x2).data());
                CpxNumVec val(NG*NG, false, NOW(k1,k2)(x1,x2).data());
                zgemv(1, evl, den, 0, val);
            }
        }
        
        for(int ell=tree_middle; ell>=EL; ell--)
        {
            int nk = pow2(ell);	int nx = N/nk;
            int kB = N/nk;	int xB = N/nx;
            ...
            for(int k2=k2stt/kB; k2<k2end/kB; k2++)
            {
                ...
                for(int x2=0; x2<nx; x2++)
                {
                    ...
                    if(ell!=EL)
                    {
                        int q1 = int(floor(k1/2));
                        int q2 = int(floor(k2/2));
